...
contract Tweeter {
    ...
    function getTweet(address _authorAddress, uint256 _id) 
        external view returns (string memory content, uint256 timestamp, uint256 likes, uint256 commentsCount) 
    {
    ...
    function _removeLike(address _userAddress, address _authorAddress, uint256 _tweetId) private {
        for (uint i = 0; i < tweets[_authorAddress][_tweetId].likers.length; i++) {
            if (tweets[_authorAddress][_tweetId].likers[i] == _userAddress) {
                tweets[_authorAddress][_tweetId].likers[i] = 
                    tweets[_authorAddress][_tweetId].likers[tweets[_authorAddress][_tweetId].likers.length - 1];
    ...
    function getSpecificComment(address _authorAddress, uint256 _tweetID, uint256 _commentIndex)
        external view returns (address commenter, string memory content, uint256 timestamp)
    {
        require(_tweetID < tweets[_authorAddress].length, "Tweet does not exist");
        require(_commentIndex < tweets[_authorAddress][_tweetID].comments.length, "Comment does not exist");
        
        Comment memory comment = tweets[_authorAddress][_tweetID].comments[_commentIndex];
